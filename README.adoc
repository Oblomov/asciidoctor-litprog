...
substitutions in source blocks are not supported in the tangling phase,
so things like author and version information needs to be duplicated
between the Asciidoctor document metadata and the source blocks
(see the <<licensing-statement,Licensing statement>> in this document as an example).
