...
.Declare...
[source]
@chunk_refs = Hash.new { |hash, key| hash[key] = [] }
@chunk_backrefs = Hash.new { |hash, key| hash[key] = [] }

Updates to `@chunk_refs` and `@chunk_backrefs` are abstracted by the `add_chunk_ref` function:

.Support...
[source]
----
def add_chunk_ref includer, includer_block_id, included
  @chunk_refs[includer].push included
...
preceding/following blocks in the same list, and/or if another chunk is including this one:
...
if @chunk_backrefs.key? chunk_title
  for inc in @chunk_backrefs[chunk_title]
...
Chunk names in the DOT file have to be properly quoted,
and we limit their length to avoid the nodes in the graph from being too long.
This is achieved through two support functions that implement a trivial line-wrapping scheme
...
def quote_for_dot text
  '"' + limit_line_length(text, 33).gsub('"', '\"') + '"'
end
----

The connections between the graphs are simply obtained iterating over the
<<chunkdef,chunk references>>, making sure that the chunk names are properly quoted:

.Output DOT connections
[source]
----
@chunk_refs.each { |chunk, refs|
  quoted_chunk = quote_for_dot chunk
  refs.each { |ref|
    quoted_ref = quote_for_dot ref
    f.puts "#{quoted_ref}:e -> #{quoted_chunk}:w"
  }
}
----

Chunks are output to the DOT file again with proper quoting,
...
@chunk_names.each { |chunk|
  quoted_chunk = quote_for_dot chunk
  if @roots.key? chunk
    f.puts "#{quoted_chunk} [fontname=\"Monospace\"]"
  else
    f.puts "#{quoted_chunk}"
  end
}
