...
We support an extension to the `aweb` syntax in the sense that the title of the chunk defined
by a source block is taken from the block title itself.

(Presently, we are actually not fully compatible with `aweb` either, since listing blocks
with explicit chunk definition(s) are not yet suported.)
...
The user can override this by setting the `literate-programming-line-template` document attribute to any string,
...
IMPORTANT: If `literate-programming-line-template` is _unset_, the default line template will be used.
...
----
:literate-programming-line-template: # %{file}:%{line}
----
...
** global setting implemented via `literate-programming-line-template` document attribute;
