...
Likewise, the custom formatter looks for `Comment::Special` tokens
and turns them into hyperlinks with a `litprog` class.

TODO: The references themselves must be fetched from the document.
  ...
  def safe_span tok, safe_val
    ...
    if special
      ...
      if m
        title = m[1]
        puts node.document
        safe_val = "&lt;&lt;<a class='litprog' href=''>#{m[1]}</a>&gt;&gt;"
      end
    end
    super
  end
