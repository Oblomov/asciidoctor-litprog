...
The chunk contents is an `Array` whose elements are either ``String``s (the actual chunk lines)
or ``Asciidoctor::Reader::Cursor``s (source line information). We currently skip the latter.
...
def recursive_tangle file, chunk_name, indent, chunk, stack
  ...
  chunk.each do |line|
    next if line.is_a? Asciidoctor::Reader::Cursor
